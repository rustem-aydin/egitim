...
export default function FilterRequiredIncompletedModules({
  modules,
  startTransition,
}: FilterModulesProps) {
  ...
  const urlValues =
    searchParams.get('requiredButNotCompletedModules')?.split(',').filter(Boolean) || []
  ...
  const handleModuleChange = (selected: Option[]) => {
    ...
    if (values.length > 0) {
      params.set('requiredButNotCompletedModules', values.join(','))
    } else {
      params.delete('requiredButNotCompletedModules')
    }
