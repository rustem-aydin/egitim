...
export default function FilterInCompletedModules({ modules, startTransition }: FilterModulesProps) {
  ...
  const urlValues = searchParams.get('inCompletedModules')?.split(',').filter(Boolean) || []
  ...
  const handleModuleChange = (selected: Option[]) => {
    ...
    if (values.length > 0) {
      params.set('inCompletedModules', values.join(','))
    } else {
  ...
  return (
    <div className="space-y-2">
      <Label>Tamamlanmayan Modüller</Label>
      <MultipleSelector
