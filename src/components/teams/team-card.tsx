...
import { Building, ClipboardList, Shield, Users } from 'lucide-react'
...
export const TeamCard = ({ team }: { team: Team }) => {
  ...
  return (
    <MotionCard>
      <Card
        className="w-full max-w-md relative overflow-hidden backdrop-blur-sm bg-gradient-to-br from-background to-muted/20"
        style={{
      ...
      >
        ...
        <CardContent className="px-6 pt-2 space-y-4 relative z-10">
          <div
            className="relative rounded-xl overflow-hidden border p-4 space-y-4"
              ...
              <div className="flex flex-wrap gap-2">
                {groups.length > 0 ? (
                  <>
                    ...
                    {groups.length > 5 && (
                      <Badge variant="outline">+{groups.length - 5} daha</Badge>
                    )}
                  </>
                ) : (
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {requiredModules.length > 0 ? (
                  <>
                    {requiredModules.slice(0, 3).map((module: any) => (
                      <BadgeModule code={module?.code} key={module.id} />
                    ))}
                    {requiredModules.length > 3 && (
                      <Badge variant="outline">+{requiredModules.length - 3} daha</Badge>
                    )}
                  </>
                ) : (
                  <p className="text-xs text-muted-foreground">Atanmış zorunlu modül yok.</p>
                )}
              </div>
            </div>
          </div>
